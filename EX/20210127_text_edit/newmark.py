...
def newmark_linear_acc():
    ...
    for i in range(0, len(Tn)):
        dis_max, pv, pa = calc_max_respense(Wn[i], KSEE)
...
# --------------------------------------------------------------------------
# name       : calculate the max response 
# description: the results are max dispalcement, PSV and PSA
#              
#
# data       : 20210128
# author     : garyhsieh
# --------------------------------------------------------------------------
def calc_max_respense(_Wn, ksee):
    _delta_t = 0.005
    #_delta_t = 0.05
    #_delta_t = 0.1
    ...
    for j in np.arange(0, len(time)):
        ...
        P_up.append(- k * up_acc[j] / _Wn ** 2)
        P_NS.append(- k * NS_acc[j] / _Wn ** 2)
        P_EW.append(- k * EW_acc[j] / _Wn ** 2)
    

    # step 1: calc initial u0_dprime
    _ui_dprime = - EW_acc[0] - 2 * ksee * _Wn * _ui_prime - (_Wn ** 2) * _ui

    # step 2: calc a1, a2, a3
    a1 = k * ( (6 / _delta_t ** 2) / _Wn ** 2 + (6 * ksee / _delta_t) / _Wn )
    a2 = k * ( (6 / _delta_t) / _Wn ** 2 + (4 * ksee) / _Wn )
    a3 = k * ( 2 / _Wn ** 2 + _delta_t * ksee / _Wn )
    k_hat = k * ( 1 + a1 / k)
    ...
    for l in np.arange(0, len(P_EW) - 1):
        ...
        wui.append(_Wn * _ui_next)
        dwui.append( (_Wn ** 2) * _ui_next )
    ...
    _PSV = max(list(temp_wui))

    temp_dwui = map(abs, dwui)
    #PSA.append(max(list(temp_dwui)))
    _PSA = max(list(temp_dwui))

    return _umax, _PSV, _PSA
